...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    const stale = keys.filter((k) => k.startsWith('ghostline-') && !k.startsWith(VERSION));
    await Promise.all(stale.map((k) => caches.delete(k)));
    await self.clients.claim();
    // Deliberately not navigating the open pages from here. A worker cannot
    // tell whether someone is standing in a field halfway through a match, and
    // reloading them mid-game to save a tap is a bad trade. The page decides
    // for itself, in main.js, when it is safe.
  })());
});
